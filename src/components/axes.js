...
function YAxis (props) {
    ...
    const tickWidth = 10; // 设置一个固定的刻度宽度
        ...
        <g>
            ...
            {yScale.domain().map(tickValue =>
                <g key={tickValue+'B'} transform={`translate(${0}, ${yScale(tickValue)})`}>
                    <line x1={-5} x2={0} y1={tickWidth/2} y2={tickWidth/2} stroke='black' />
                    <text style={{textAnchor: 'start', fontSize:'10px' }} x={-offsetX+10} y={tickWidth/2}>
                        {tickValue}
                    </text>
                </g>
            )}
